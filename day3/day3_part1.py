...
n_bits = len(open("day3/input.txt", "r").readline().rstrip())
set_bit_counts = [0] * n_bits

n_lines = 0
for line in input_file.readlines():
    n_lines += 1
    for i_char in range(n_bits):
        if line[i_char] == "1":
            # Count number of 1s in each bit position
            set_bit_counts[i_char] += 1

gamma_rate = ""
epsilon_rate = ""
for x in set_bit_counts:
    if x > n_lines / 2:
        # If count of 1s is greater than half the num lines. 1 is most common.
        gamma_rate += "1"
        epsilon_rate += "0"
    else:
        gamma_rate += "0"
